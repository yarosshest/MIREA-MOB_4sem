...
class Work95 : AppCompatActivity() {
    var states = ArrayList<State>()
    var countriesList: ListView? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_work95)
        setInitialData()
        // получаем элемент ListView
        countriesList = findViewById<View>(R.id.countriesList) as ListView?
        // создаем адаптер
        val work95StateAdapter = Work95StateAdapter(this, R.layout.work_95_list_item, states)
        // устанавливаем адаптер
        countriesList?.adapter = work95StateAdapter
